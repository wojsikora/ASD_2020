""" Algorytm polega na:
1) Zrobieniu tablicy i przypisywaniu wartosci z tuple i dodatkowe dodanie miejsca dla wartosci profit=(val/curr_weight), O(n)
...
Algorytm jest zachłanny, bo jesteśmy pewni że biorąc element który bierzemy uzyskujemy największy profit (I nie będzie sytuacji że jakiś z małym profitem (val/curr_weight) tak naprawdę jest lepszy dla nas: (Np (val=2,weight=2),(val=3,weight=4): i mamy do uzupełnienia 5 kg, w tym przypadku powyższy algorytm (po usunieciu fractional part) zwroci val=2, bo profit z pierwszego jest większy niż profit z drugiego (Przedstawiłem dyskretną warjację tego problemu). Czyli w naszym problemie, w którym możemy brać części przedmiotów takiej sytuacji być nie może"""
